                                                        ...
                                                        $row_sched= mysqli_fetch_assoc($result_sched);
                                                                ...
                                                                *  
                                                                ...
                                                                if (isset($_POST['name_wthPay'])) {
                                                                    ...
                                                                    if($leave_type == 'Vacation Leave'){
                                                                        if($minusVacationCredits < 0 ){
                                                                            header("Location: ../../leavereq.php?error=You cannot apply request from the range date provide. Lack of credits for Vacation Leave");
                                                                             ...
                                                                             VALUES('$empname', '$leave_type', '$leave_period', '$str_date', '$end_date', '$reason_txt', '$escaped_contents','With Pay', 'Approved', '0.5')";

                                                                             if(mysqli_query($conn,$sql)){
                                                                                $deductSql = "SELECT * FROM leaveinfo_tb WHERE `col_empID` = '$employeeID' ";
                                                                                $deductResult = mysqli_query($conn, $deductSql);
                                                                                $deductRow = mysqli_fetch_assoc($deductResult);

                                                                                $vleave = $deductRow['col_vctionCrdt'];
                                                                                
                                                                                $vleaveUpdate  = $vleave - 0.5;

                                                                                $updateDeduct = "UPDATE leaveinfo_tb SET `col_vctionCrdt` = $vleaveUpdate WHERE col_empID = $employeeID ";

                                                                                $updateDeductRun =  mysqli_query($conn, $updateDeduct);
                                                                    ...
                                                                    $sql = "INSERT into applyleave_tb(`col_req_emp`, `col_LeaveType`, `col_LeavePeriod`, `col_strDate`, `col_endDate`, `col_reason`, `col_file`, `col_PAID_LEAVE`, `col_status` , `col_credit`) 
                                                                    VALUES('$empname', '$leave_type', '$leave_period', '$str_date', '$end_date', '$reason_txt', '$escaped_contents','Without Pay', 'Approved', '0')";
                            ...
                            if($leave_type == 'Vacation Leave'){
                                if($minusVacationCredits < 0 ){
                                    header("Location: ../../leavereq.php?error=You cannot apply request from the range date provide. Lack of credits for Vacation Leave");
                                        ...
                                        #sql query to insert into database
                                        $sql = "INSERT into applyleave_tb(`col_req_emp`, `col_LeaveType`, `col_LeavePeriod`, `col_strDate`, `col_endDate`, `col_reason`, `col_file`, `col_PAID_LEAVE`, `col_status`, `col_credit`) 
                                        VALUES('$empname', '$leave_type', '$leave_period', '$str_date', '$end_date', '$reason_txt', '$escaped_contents','With Pay', 'Approved', '$working_days_ofLEave')";
