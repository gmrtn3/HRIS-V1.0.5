...
//-------------------------------FOR FETCHING HOLIDAY---------------//
        ...
        $checker_holiday = "SELECT * FROM schedule_list";
        ...
        if(mysqli_num_rows($result_holiday) <= 0 ){      
            ...
            if ($data['meta']['code'] == 200) {
                ...
                foreach ($data['response']['holidays'] as $holiday) {
                    ...
                    $start_datetime = $dateTime->format('Y-m-d');
                
                    $end_datetime = $dateTime->format('Y-m-d');
                    ...
                    $query = "SELECT * FROM `schedule_list` WHERE `title` =  ? AND `start_datetime` = ? ";
                    $stmt_check = mysqli_prepare($conn, $query);
                    mysqli_stmt_bind_param($stmt_check, 'ss', $title, $start_datetime);
                    ...
                    if (mysqli_stmt_num_rows($stmt_check) == 0) {
                        ...
                        mysqli_stmt_bind_param($stmt, 'ssss', $title, $isWorkingDayHoliday, $start_datetime, $end_datetime);
            ...
            $checker_year = "SELECT YEAR(start_datetime) as year FROM schedule_list ORDER BY start_datetime DESC";
