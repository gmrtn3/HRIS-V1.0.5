<?php
include '../../config.php';
if (isset($_POST['yes_download_undertime'])) {
   $select_tableid = $_POST["table_id_undertime"];
   $select_tablename = $_POST["table_name_undertime"];


   // Step 2: Retrieve blob data from database
        $sql = "SELECT `file_attachment` FROM `undertime_tb` WHERE `id` = ?";
